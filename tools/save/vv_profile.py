...
# is her bespoke Select-Skater portrait ss_vv (a white full-body silhouette matching the
# stock pros — mods/src/vv-skater/assets/ss_vv.img.xbx, generated by build_vv_portrait.py
# and loaded via the ss_vv line inject_sprites() adds to sprites.qb).
ENTRY = '''    {
        display_name="Violet Vandal"
        select_icon=ss_vv
        first_name="Violet"
...
# The Select Skater screen only draws a portrait it has loaded: sprites.qb's
# load_select_skater_textures_to_main_memory holds a hard-coded array of the ss_* textures
# that LoadTexture pulls from Data/images/MainmenuSprites/. ss_vv must be in it, or the
# SpriteElement finds no texture. (The .img.xbx itself ships loose via the build.)
SPRITE_LINE = '        "MainmenuSprites/ss_vv" '


def inject_sprites(lines):
    """Given sprites.ns as a list of lines, add ss_vv to the select-skater texture load
    array so Violet Vandal's portrait is available on the Select Skater screen. Idempotent."""
    if any('MainmenuSprites/ss_vv' in l for l in lines):
        return lines
    out = list(lines)
    si = next(i for i, l in enumerate(out)
              if l.strip().startswith('script load_select_skater_textures_to_main_memory'))
    ti = next(i for i in range(si, len(out)) if out[i].strip().startswith('textures = ['))
    close = next(i for i in range(ti + 1, len(out)) if out[i].strip().startswith(']'))
    out.insert(close, SPRITE_LINE)
    return out
